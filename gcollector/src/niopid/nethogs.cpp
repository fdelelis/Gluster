...
/* II: Nethogs structures treats a PID as a signed integer */
int watchpid = 0;
char *search = NULL;
...
static void help(void)
{
	...
	std::cerr << "usage: nethogs [-V] [-b] [-d seconds] [-t] [-p] [-P] [-s] [-i] [device [device [device ...]]]\n";
	std::cerr << "          -V          : prints version.\n";
	std::cerr << "          -d <delay>  : delay for update refresh rate in seconds. default is 1.\n";
	std::cerr << "          -t          : non interactive trace mode\n";
	...
	std::cerr << "The following options apply only when -t is passed:\n";
	std::cerr << "          -P <pid>    : only display information about [pid]\n";
	std::cerr << "          -s <string> : only display information about processes that match [string]\n";
	std::cerr << "          -i <num>    : iterate <num> times sampling for [-t time], and exit\n";
	std::cerr << std::endl;
	std::cerr << "When nethogs is running interactively, press:\n";
	std::cerr << " q: quit\n";
	std::cerr << " m: switch between total and kb/s mode\n\n";
	std::cerr << "If no [device] is specified, eth0 is used\n";
...
int main (int argc, char** argv)
{
	...
	int opt;
	while ((opt = getopt(argc, argv, "VhbtpP:s:i:d:")) != -1) {
		switch(opt) {
			case 'V':
				versiondisplay();
				exit(0);
			...
			case 'p':
				promisc = 1;
				break;
			/* II: Added for string / pid matching */
			case 'P':
				watchpid = atoi(optarg);
				do_filter = true;
				break;
			case 's':
				...
				do_filter = true;
				break;
			/* II: End of string / pid matching */
			/* II: Added for iterations */
			case 'i':
				iterations = atoi(optarg);
				break;
			/* II: End of iterations */
			case 'd':
	...
	/*II: Don't immediately refresh while in trace mode */
	if (tracemode)
		needrefresh = false;
	...
	while (1)
	{
		...
		{
			do_refresh();
			/* II: Check to see if we've iterated the requested number of times.
			 * setting iterations_complete high will cause the main loop to exit
		     */
			if (iterations > 0) {
				current_iteration ++;
