...
/* II: Globals used in lieu of injecting additional arguments into methods */
...
void Line::show (int row)
{
	...
	if (DEBUG || tracemode)
	{
		/* II: Don't print information for processes we have yet to identify */
		if (m_pid == 0)
			return;
		
		/* II: Implement PID and string matching. If filtering, don't bother inserting non-matches into lines[] */
		if (do_filter) {
			if (watchpid > 0) {
				if (m_pid != watchpid)
					return;
			...
			if (search != NULL) {
				if (strcmp(m_name, search) != 0)
					return;
			}
		}
		/* II: End PID and string matching */
		/* II: Format of output changed to tab delimited, print device name first */
		std::cout << devicename << "\t" << m_name << "\t" << m_pid << "\t" << m_uid << "\t" << sent_value << "\t" << recv_value << std::endl;
		return;
	}
...
void do_refresh()
{
	...
	while (curproc != NULL)
	{
		...
		else
		{
			...
			/* II: If we're iterating [iterations] times, we want the aggregate value,
			 * so we select VIEWMODE_TOTAL_KB
			 */
			if (iterations > 0)
				viewMode = VIEWMODE_TOTAL_KB;
			/* II: End iteration adjustment */
			...
			//free (pwuid);
#endif
			/* II: The next assertion seems useless, as pid is initialized to 0, and no error
			 * causes it to be signed
			 */
	...
	for (i=0; i<nproc; i++)
	{
		// II_TODO - Filter the display here
		lines[i]->show(i);
		recv_global += lines[i]->recv_value;
		sent_global += lines[i]->sent_value;
		delete lines[i];
	}
	/* II: Only print out 'unknown' connections in bughunt mode, not trace mode */
	if (DEBUG && ! tracemode) {
